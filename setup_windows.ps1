# =============================================================================
#  setup_windows.ps1 — Windows Setup & Launcher for Media Downloader
# =============================================================================
#
...
#   10. Launches the Streamlit app and opens it in the default browser
#
#  REQUIREMENTS
#    - Windows 10 (build 1809+) or Windows 11
#    - PowerShell 5.1 or PowerShell 7+ (both are supported)
#    - An internet connection for first-time dependency installation
#    - Administrator rights are NOT required for winget installs
#      (Chocolatey may require an elevated session for some packages)
#
...
function Write-Banner {
    ...
    Write-Host "║     ⬇  Media Downloader — Windows Setup & Launcher          ║" -ForegroundColor Cyan
    Write-Host "╚══════════════════════════════════════════════════════════════╝" -ForegroundColor Cyan
    Write-Host ""
}
...
if ($SkipPkgManager -eq "1") {
    ...
    elseif (Test-Command "scoop") {
        ...
        Write-Warn "System dependencies (Python, git, ffmpeg) must be installed manually."
...
if ($SkipPkgManager -eq "1" -or $null -eq $PkgManager) {
    ...
    if (-not (Test-Command "ffmpeg")) {
        Write-Info "ffmpeg not found. Installing via $PkgManager…"
        switch ($PkgManager) {
            "winget" {
                Invoke-WithRetry -MaxAttempts 3 -DelaySeconds 10 -Description "Install ffmpeg" -Command {
        ...
        if (Test-Command "ffmpeg") {
            ...
            Write-Warn "ffmpeg installation may require a new terminal session to take effect."
            Write-Warn "MP3 extraction and high-quality video merging will not work until then."
        }
    } else {
...
if (-not (Test-Path $DownloadsDir)) {
    ...
    Write-Ok "downloads\ directory exists."
}
